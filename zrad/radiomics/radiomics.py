...
class Radiomics:
    ...
    def _calc_texture_features(self):
        ...
        glszm = GLSZM(image=self.patient_intensity_mask.array.T,
                                              slice_weight=self.slice_weighting, slice_median=self.slice_median)
        if self.aggr_dim == '3D':
            glszm.calc_glsz_3d_matrices(self.patient_morphological_mask.array.T)
        ...
        gldzm = GLDZM(image=self.patient_intensity_mask.array.T,
                                              slice_weight=self.slice_weighting, slice_median=self.slice_median)
        if self.aggr_dim == '3D':
            gldzm.calc_gldz_3d_matrices(self.patient_morphological_mask.array.T)
            ...
            gldzm.calc_gldz_2d_matrices(self.patient_morphological_mask.array.T)
            if self.aggr_dim == '2.5D':
